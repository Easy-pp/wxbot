...
async function getContactTextReply(that, contact, msg) {
	...
	let result = await service.filterFriendMsg(
		msg,
		contactName,
		contactId,
		contactAvatar
	...
	if (result.type == "text") {
		...
		try {
			...
			if (scheduleObj.isLoop) {
				if (scheduleObj.time) {
					...
					contact.say(
						"提醒设置失败，请保证每个关键词之间使用空格分割开，并保证日期格式正确。正确格式为：“提醒(空格)我(空格)每天(空格)18:30(空格)下班回家"
					);
				}
			} else {
				...
				if (isTime) {
					...
					await lib.delay(2000);
					contact.say(
						"提醒设置失败，日期格式不正确。正确格式为：“提醒(空格)我(空格)18:30(空格)下班回家” 或“提醒(空格)我(空格)2019-10-01 8:30(空格)还有两天就是老婆生日，要准备一下了”"
					);
				}
			}
			return "";
		...
		if (typeof result.content === "object") {
			if (result.content.type === "file") {
				let fileObj = FileBox.fromDataURL(
					"data:text/plain;base64," + result.content.src,
					contactName + contactId + ".jpg"
				);
				return fileObj;
